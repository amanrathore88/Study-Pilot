...
def build_timetable_table(rows, S):
    ...
    for i, row in enumerate(rows):
        _, bg, accent = get_urgency(row["exam_date"])
        row_idx = i + 1
...
def generate_pdf(rows, summary="", output_path='timetable.pdf'):
    ...
    doc = SimpleDocTemplate(output_path, pagesize=A4, leftMargin=MARGIN, rightMargin=MARGIN, topMargin=MARGIN, bottomMargin=MARGIN, title= "studyPilot - weekly timetable",)
    ...
    doc.build(story)
    print("PDF is created successfully.")
